...
ax = fully_paid.plot.bar(x=fully_paid.index, y=fully_paid.values, rot=0)
ax.set_title('Distribution of target variable')
ax.set_ylabel('count')
ax.set_xlabel('paid.back.loam')
...
for items in cat_df.columns:
    X_train[items].fillna('NA')
    X_test[items].fillna('NA')
    X_train[items]=le.fit_transform(X_train[items])
    X_test[items]=le.transform(X_test[items])
